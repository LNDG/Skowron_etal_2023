...
cd('/home/mpib/LNDG/Entscheidung2/analysis/trial-wise_PLS')
...
cond = {'s1','s2','s3','s4','s5','grid','dec'}; %note: order needs to comply to temporal order in trial! Also naming convention needs to be according to the data structure.
...
for c = 1:length(cond)
    for sj = 1:length(ID)
        for run = 1:length(RunID)
          ...
          for i = 1:length(onsets) 
            ...
            if (strcmp('s1',cond{c}) || strcmp('s2',cond{c}) || strcmp('s3',cond{c}) || strcmp('s4',cond{c}) || strcmp('s5',cond{c}))
                durations(i) = data(sid).trial(i).timing.(['time_' cond{c+1}]) - onsets(i); % note: durations including ISI!
            elseif (strcmp('grid',cond{c}))
        ...
        OutDir='/home/mpib/LNDG/Entscheidung2/analysis/trial-wise_PLS/timings/';
