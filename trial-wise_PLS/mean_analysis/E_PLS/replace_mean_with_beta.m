function replace_mean_with_beta
...
pn.root = '/home/mpib/LNDG/Entscheidung2/analysis/trial-wise_PLS/mean_analysis/results/allCond/';
pn.meanFiles = [pn.root, 'mean_datamats/'];
pn.sdFiles = [pn.root, 'beta_datamats/'];
pn.sdniftis = '/home/mpib/LNDG/Entscheidung2/analysis/trial-wise_PLS/mean_analysis/data/mean_niftis_allCond/';
...
SessionID={'1' '2' '3' '4' '5' '6' '7'}; % If no sessions, leave completely empty as a 0x0 cell
% s1 s2 s3 s4 s5 grid dec

% Load common set of coordiants
load (['/home/mpib/LNDG/Entscheidung2/analysis/trial-wise_PLS/mean_analysis/E_PLS/coords_EVAL.mat'], 'final_coords');
...
for indID = 1:numel(SubjectID)
    ...
    a.session_info.datamat_prefix   = (['beta_', SubjectID{indID}]); % SD PLS file name; _Bfmirsessiondata will be automatically appended!
    ...
    for indCond = 1:numel(SessionID)
        fname = dir([pn.sdniftis, SubjectID{indID} '/con_000' SessionID{indCond} '.nii' ]);
        fname = fname.name;
