...
Outdir="/home/mpib/LNDG/Entscheidung2/analysis/trial-wise_PLS/mean_analysis/data/mean_niftis_allCond"
DataPath="/home/mpib/LNDG/Entscheidung2/analysis/trial-wise_PLS/mean_analysis/data/spm_spec_allCond"
...
for sub in ${SubjectID}; do
		...
		#for sc in ${samplesID}; do
				...
				if [ ! -d ${Outdir}/${sub}/ ]; then	
					...
					echo "#SBATCH --job-name GLM_con_${sub}" 	>> job # Job name 
					...
					echo "#SBATCH --mem 5GB" 								>> job # Books 10gb RAM for the job 
					echo "#SBATCH --mail-type NONE" 										>> job # Email notification on abort/end, use 'n' for no notification 
					echo "#SBATCH --output /home/mpib/LNDG/Entscheidung2/analysis/trial-wise_PLS/mean_analysis/log/slurm-%j.out" 							>> job # Write output log to group log folder
					echo "#SBATCH --workdir ${Outdir}/${sub}" 							>> job # working directory
					
				
					spm_file=${DataPath}/${sub}_allCond_con_model.mat
