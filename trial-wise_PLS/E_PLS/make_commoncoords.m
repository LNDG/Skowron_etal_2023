...
SessionID={'s1' 's2' 's3' 's4' 's5' 'grid' 'dec'};

%% Initiate paths
BASEPATH=(['/home/mpib/LNDG/Entscheidung2/']);
DATAPATH=([BASEPATH 'analysis/trial-wise_PLS/data/sd_niftis_allCond']);
SAVEPATH=([BASEPATH, 'analysis/trial-wise_PLS/E_PLS/coords_EVAL.mat']);
...
for i = 1:numel(SubjectID)
        for j = 1:numel(SessionID)
            
            %% Set image name/path
            NIIPATH=([DATAPATH '/' SubjectID{i} '/' SessionID{j} '/']);
            NIINAME=dir([NIIPATH SubjectID{i} '_allCond_sd_' SessionID{j} '_sd.nii']);
