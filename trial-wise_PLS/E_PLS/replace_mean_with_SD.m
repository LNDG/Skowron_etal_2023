...
pn.root = '/home/mpib/LNDG/Entscheidung2/analysis/trial-wise_PLS/results/allCond/';
...
SessionID={'s1' 's2' 's3' 's4' 's5' 'grid' 'dec'}; % If no sessions, leave completely empty as a 0x0 cell

% Load common set of coordiants
load (['/home/mpib/LNDG/Entscheidung2/analysis/trial-wise_PLS/E_PLS/coords_EVAL.mat'], 'final_coords');
...
for indID = 1:numel(SubjectID)
    ...
    for indCond = 1:numel(SessionID)
        fname = dir([pn.sdniftis, SubjectID{indID} '/' SessionID{indCond} '/' SubjectID{indID} '_allCond_sd_' SessionID{indCond} '_sd.nii' ]);
        fname = fname.name;
        
        img = double(S_load_nii_2d([pn.sdniftis, SubjectID{indID} '/' SessionID{indCond} '/' fname])); clear fname;
